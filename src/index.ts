...
class LexofficeAdapter implements Adapter {
  /**
  ...
  public async getContacts(config: Config): Promise<Contact[]> {
    const { apiKey } = LexofficeAdapter.validateAndReturnRequiredConfigKeys(
      config
    );
    try {
  ...
  public async handleOAuth2Callback(
    ...
    if (req.query.error) {
      throw new Error("Access denied to Lexoffice");
    }
